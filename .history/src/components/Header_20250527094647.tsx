    ...
    <header className={`fixed w-full transition-all duration-300 bg-white shadow-md ${isScrolled ? 'py-2' : 'py-4'}`} style={{ zIndex: 9999 }}>
      <div className="container mx-auto px-4">
        <nav className="flex items-center justify-between relative">
          ...
          <div 
            className={`lg:hidden flex items-center gap-x-4 ${isRTL ? 'flex-row-reverse' : ''}`}
            style={{ zIndex: 9999 }}
          >
            <div className="relative" style={{ zIndex: 9999 }}>
              <LanguageSelector />
        ...
        {isMenuOpen && (
          <div className="fixed inset-0" style={{ zIndex: 9998 }}>
            ...
            <div
              className={`absolute top-[72px] w-[90vw] max-w-xs bg-white rounded-xl shadow-2xl p-6 left-4`}
            >
