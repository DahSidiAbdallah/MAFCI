        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {projectsData.map((project) => (
            ...
            >
              ...
              />
              <div className={`absolute inset-0 bg-gradient-to-t from-secondary/90 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6 ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <div className={`flex items-center ${isRTL ? 'justify-end space-x-reverse' : 'justify-between'}`}>
                  <span className={`bg-primary text-white text-xs px-3 py-1 rounded-full ${isRTL ? 'ml-2' : 'mr-2'}`}>
