...
const Header: React.FC = () => {
  ...
  return (
    <header className={`fixed w-full z-40 transition-all duration-300 bg-white shadow-md ${isScrolled ? 'py-2' : 'py-4'}`}>
      <div className="container mx-auto px-4">
        <nav className="flex items-center justify-between">
          ...
          <div className={`hidden lg:flex items-center ${isRTL ? 'order-2 justify-end flex-1 mx-8' : 'order-2 justify-start flex-1 mx-8'}`}>
            <div className={`flex items-center gap-x-8`}>
              ...
              <a href="#products" className="text-secondary font-medium hover:text-primary transition-colors whitespace-nowrap">
                {t('nav.products')}
              </a>
            </div>
          </div>
          ...
          {/* Mobile Menu Button */}
          <div className={`lg:hidden flex items-center gap-x-4 ${isRTL ? 'order-1' : 'order-3'}`}>
            <LanguageSelector />
            <button
              onClick={toggleMenu}
        ...
        {isMenuOpen && (
          <div className="fixed inset-0 z-50 lg:hidden">
            ...
            <div
              className={`absolute top-4 w-[90vw] max-w-xs bg-white rounded-xl shadow-2xl p-6 
                ${isRTL ? 'left-4' : 'right-4'}`}
            >
              <button
                onClick={toggleMenu}
                className={`absolute top-3 text-gray-500 hover:text-primary 
              ...
              <nav className={`flex flex-col gap-y-5 mt-2 mb-6 ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                <a href="#products" className="text-lg text-gray-800 font-medium hover:text-primary transition-colors" onClick={toggleMenu}>
                  {t('nav.products')}
                </a>
              </nav>
